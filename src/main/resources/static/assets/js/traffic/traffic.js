    ...
    function clearMap() {
        ...
        departureStation.addEventListener("change", (e) => {
            ...
            sx = parseFloat(option.dataset.longitude);
            sy = parseFloat(option.dataset.latitude);
        ...
        arrivalStation.addEventListener("change", (e) => {
            const option = e.target.selectedOptions[0];
            ex = parseFloat(option.dataset.longitude);
            ey = parseFloat(option.dataset.latitude);
            searchPubTransPathAJAX();
            clearMap();
            if(sx === ex  && sy===ey){
    ...
    function searchPubTransPathAJAX() {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                document.getElementById('total-stations').textContent = jsonResponse.result.path[0].info.totalStationCount || "정보 없음";
                document.getElementById('total-MetroTime').textContent = jsonResponse.result.path[0].info.totalTime || "정보 없음";
            }
        };
    }
    ...
    function callMapObjApiAJAX(mapObj) {
        var xhr = new XMLHttpRequest();
        var url = "https://api.odsay.com/v1/api/loadLane?mapObject=0:0@" + mapObj + "&apiKey=avqjby448YJDDqi4buwo5S69Uz5sf0nc3aEMdXWFzsQ";
        xhr.open("GET", url, true);
        xhr.send();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
