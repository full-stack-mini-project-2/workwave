        ...
        function initTeam() {
            ...
            $http.get('/api/todos/team/aTeamTodos/' + departmentId)
                .then(function(response) {
                    ...
                    // 문자열 true/false를 boolean으로 변환
                    $scope.teamTasks.forEach(task => {
                        task.teamTodoStatus = task.teamTodoStatus === 'true' ? 'true' : 'false';
                    });
                })
...
<%--            <td>{{$index + 1}}</td>--%>
<%--            <td ng-click="editTeamTask(teamTask)">--%>
<%--                <span ng-show="!teamTask.editing" ng-class="{'complete': teamTask.teamTodoStatus}">{{teamTask.teamTodoContent}}</span>--%>
<%--                <input ng-show="teamTask.editing" type="text" ng-model="teamTask.teamTodoContent" ng-blur="updateTeamTask(teamTask)" ng-keypress="handleKeyPressTeamTask($event, teamTask)">--%>
<%--            </td>--%>
<%--            <td>--%>
<%--                <input type="checkbox" ng-model="teamTask.teamTodoStatus" ng-change="updateTeamTask(teamTask)">--%>
<%--            </td>--%>
<%--            <td>--%>
<%--                <button class="btn btn-danger btn-sm" ng-click="deleteTeamTask($index)">Delete</button>--%>
<%--            </td>--%>
...
<%--    // status 체크박스가 true일 때와 false일 때의 데이터 가져오기--%>
<%--</script>--%>
