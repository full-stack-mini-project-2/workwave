...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <tr ng-repeat="task in tasks" ng-class="{'table-success': task.todoStatus, 'table-light': !task.todoStatus}">
            <td>{{$index + 1}}</td>
            <td>
                <!-- 클릭 시 수정 가능한 입력 필드로 변경 -->
                <span ng-hide="task.editing" ng-click="edit(task)">{{task.todoContent}}</span>
                <input ng-show="task.editing" ng-model="task.todoContent" type="text" class="form-control" ng-keypress="handleKeyPress($event, task)">
            </td>
            <td>{{task.todoStatus ? 'Completed' : 'In progress'}}</td>
            <td>
<%--                <button ng-click="update(task)" ng-hide="task.editing">Update</button>--%>
                <button ng-click="cancelEdit(task)" ng-show="task.editing">Cancel</button>
...
<%--    <h4>Edit Task</h4>--%>
<%--    <div class="input-group mb-3">--%>
<%--        <input ng-model="editingTask.todoContent" type="text" class="form-control" ng-keypress="handleKeyPress($event, editingTask)">--%>
<%--        <div class="input-group-append">--%>
